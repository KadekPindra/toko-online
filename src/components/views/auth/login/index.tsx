  ...
  return (
    <AuthLayout label="Hi Visitors, Welcome back to" title="Online Shop Website">
      <form onSubmit={handleSubmit} className="flex w-full flex-col gap-2">
        ...
        <div className="w-full flex justify-center ">{error && <p className="text-red-500 text-sm">{error}</p>}</div>
        <Button type="submit" color="default" variant="contained" className="mt-5 py-2">{isLoading ? "Loading..." : "Login"}</Button>
        ...
        <div className="flex items-center w-full justify-center mt-2">
          <p className="text-sm">
            ...
            <Link href={"/auth/register"}><span className="text-blue-600">Sign Up</span></Link>
          </p>
        </div>
      </form>
    </AuthLayout>
